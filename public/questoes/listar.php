...
<div class="container mt-4">
  ...
  <table class="table table-bordered table-striped align-middle">
    ...
    <tbody>
      ...
      <form id="form <?= $p["Id"] ?>" method="POST" action="/API-Provas-main/questoes/alterar&id=<?=$p["Id"]?>">
      <tr id="<?= $p["Id"] ?>">
        <td><?= $p["Id"] ?></td>
        <td>
      ...
      <form method="POST" action="/API-Provas-main/?param=questoes/inserir">
      <tr class="table-form-row">
        ...
        <td colspan="2"><input type="submit" class="btn btn-primary" value="Enviar"></td>
      </tr>
      </form>
    </tbody>
  </table>
  <form method="POST" action="/API-Provas-main/?param=questoes/gerarProva" class="row g-3">
    <div class="col-md-4">
...
<script>
  ...
  function deletarQuestao(id) {
    if (confirm('Tem certeza que deseja deletar essa questão?')) {
      ...
      form.action = `/API-Provas-main/?param=questoes/deletar&id=${id}`;
      form.submit();
    }
  }
</script>
